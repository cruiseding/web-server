...
<nav class="breadcrumb">
	...
	<span class="c-gray en">&gt;</span> 渠道商提现
...
function fresh_page(pageIndex) {
	...
	$.ajax({
		...
		url:"<%=path%>/ar?p0=A-bigboss-search&p1=acash_withdrawal&p2="+pageIndex+"&p5=tojson",
		async: true,
...
function jujue_money(obj,id){
	...
	layer.confirm('拒绝提现给用户？',function(index){
		var age = prompt("请输入拒绝原因","");
		if(age!=""){
		layer.confirm('确认输入以上内容？',function(index){
			$.ajax({
				type: 'POST',
				url: '<%=path%>/ar?p0=A-bigboss-mod&p1=jujue_money&p2='+id+'&p3='+age,
				success: function(data){
...
function response_money(obj,id){
	layer.confirm('确认提现给用户？',function(index){
		...
		$.ajax({
			type: 'POST',
			url: '<%=path%>/ar?p0=A-boss-mod&p1=response_money&p2='+id+'&p3=2',
			success: function(data){
				/*$(obj).parents("tr").remove();*/
				layer.msg(data,{icon:1,time:1000});
				setTimeout(function () { 
