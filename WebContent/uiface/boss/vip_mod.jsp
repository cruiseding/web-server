...
<body>
	<article class="page-container">
		<form 
			class="form form-horizontal" id="form-article-add" name="ThisForm"> <%--  action="<%=path%>/ar?a=A-boss-mod&b=vip_mod1" method="post" --%>
		<div class="row cl">
			   <input type="hidden" value="${reList[0]['id']}"  name="myid" id="myid">
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text"
						value="${reList[0]['name']}" placeholder="请输入内容" 
						id="num" name="num">
				</div>
			</div>
			...
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2">平均价格(元/天):</label>
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text"
						value="${reList[0]['day_price']}" placeholder="请输入内容" 
						id="num1" name="num1">
				</div>
			</div>
			
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2">总价格(元):</label>
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text"
						value="${reList[0]['money']}" placeholder="请输入内容" 
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					...
					<select class="input-text" style="width:150px" id="searchname2" name="searchname2">
						<c:if test="${reList[0]['is_recommand']=='0'}">
								<option value="0">不推荐</option>
								<option value="1">推荐</option>
						</c:if>
						<c:if test="${reList[0]['is_recommand'] == '1' }">
								<option value="1">推荐</option>
								<option value="0">不推荐</option>
						</c:if>
					</select>
	...
	function go(){
		...
		$.ajax({
			type:'POST',
			url: '<%=path%>/ar?a=A-boss-mod&b=vip_mod1',
			data:{myid:$("#myid").val(),num:$("#num").val(),num1:$("#num1").val(),changeprice:$("#changeprice").val(),searchname2:$("#searchname2").val()},
