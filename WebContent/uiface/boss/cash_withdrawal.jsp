...
<nav class="breadcrumb">
	...
	<span class="c-gray en">&gt;</span> 趣豆收益提现
	...
	$.ajax({
		...
		function jujue_money(obj,id){
			layer.confirm('拒绝提现给用户？',function(index){
				...
				$.ajax({
					type: 'POST',
					url: '<%=path%>/ar?p0=A-boss-mod&p1=jujue_money&p2='+id+'&p3=0',
					success: function(data){
...
function response_money(obj,id){
	layer.confirm('确认提现给用户？',function(index){
		...
		$.ajax({
			type: 'POST',
			url: '<%=path%>/ar?p0=A-boss-mod&p1=response_money&p2='+id+'&p3=0',
			success: function(data){
				/*$(obj).parents("tr").remove();*/
				layer.msg(data,{icon:1,time:1000});
				setTimeout(function () { 
...
function seek2(){
	...
	layer.confirm('确认要导出数据吗？', function(index) {
		window.open("<%=path%>/ar?p0=A-boss-search&p1=cash_withdrawal2&p2=1&p3="+startdate+"&p4="+enddate+"&p5=tojson&p6="+pp+"&p7=0&p8="+searchtext+"&p9="+searchtxtnc+"&p10="+tixian);
