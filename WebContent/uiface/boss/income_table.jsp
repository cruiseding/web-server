	  ...
	  <div class="mt-20">
			...
			<input type="text" id="d244" onclick="WdatePicker({dateFmt:'yyyy'})" class="input-text Wdate" style="width:120px;"  />
		      ...
		      <input type="text" class="input-text" style="width: 150px;" placeholder="请输入用户昵称" id="searchtxtnc" name="searchtextnc">	
		      	...
		      	<select class="input-text" style="width:150px" id="income_type" name="">
					...
					<option value="短视频收入">短视频收入</option>
					<!-- <option value="充值VIP">充值VIP</option>
					<option value="充值趣豆">充值趣豆</option>
					<option value="女神提现">女神提现</option> -->
			...
			<span id="sum1">总收入（趣豆）:${reList[0].sum}</span>
		</div>
...
function fresh_page(pageIndex) {
	...
	$.ajax({
		...
		url:"<%=path%>/ar?p0=A-boss-search&p1=income_table_search&p2="+pageIndex+"&p3=tojson&p4=&p5="+searchtext+"&p6="+searchtxtnc+"&p7="+startdate+"&p8="+enddate+"&p9="+pp+"&p10="+income_type,
		...
		success: function(data) {
			...
			for(var i = 0; i < json.length-1; i++) {
				...
				+'<td>'+json[i].type+'</td>'
				+'<td>'+Number(json[i].money).toFixed(2)+'(趣豆)</td>'
