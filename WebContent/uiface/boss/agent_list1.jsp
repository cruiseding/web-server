...
<nav class="breadcrumb">
	...
	<span class="c-gray en">&gt;</span> 渠道商列表 
...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"> <a href="javascript:;" onclick="system_category_add('渠道商添加','<%=path%>/boss/agent_add1.jsp?method=add','600','600')"  class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加渠道商</a></span> </div>	
	<div class="mt-20">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
				...
				<th width="40">渠道名称</th>
				<th width="40">渠道链接</th>
				<th width="40">总收入（元）</th>
				<th width="40">可提现收入（元）</th>
				<th width="40">提现账户</th>
				<th width="40">提现姓名</th>
				<th width="40">充值分成</th>
				<th width="40">提现分成</th>
		...
		<tbody id="list-content">
			<c:forEach var = "map" items="${reList }" varStatus="status">
				<tr class="text-c">
					<td>${status.count}</td>
					<td>${map['id']}</td>
					<td>${map['agent_username']}</td>
					<td>${map['agent_password']}</td>
					<td>${map['agent_channelcode']}</td>
					<td>${map['agent_name']}</td>
					<td>${map['agent_url']}</td>
					<td>${map['totle_money']}</td>
					<td>${map['ablew_money']}</td>
					<td>${map['agent_pay']}</td>
					<td>${map['agent_payname']}</td>
					<td>${map['fencheng_u']}</td>
					<td>${map['fencheng_v']}</td>
					<td>
					<button type="button" class="btn btn-success radius" onclick="system_category_add('渠道商修改','<%=path%>/ar?a=A-boss-mod&b=agent_modbf1&c=${map['id']}','600','600')"   name=""><i class="Hui-iconfont">&#xe60c;</i>修改</button>
					<br><br>
					<button type="button" class="btn btn-danger radius"  onclick=shanchu('${map['id']}')   name=""><i class="Hui-iconfont">&#xe609;</i>删除</button>
					
					
					</td>
					<%-- <td><a href="javascript:;"
					onclick="system_category_edit1('变更代理商等级','<%=path %>/rp?p0=A-boss-mod&p1=agentlevel_mod1&p2=${map['id'] }&p3=0','600','400')"
					style="text-decoration: none;color:blue;" class="ml-5">修改</a> </td>--%>
...
function fresh_page(pageIndex){
	...
	$.ajax({
		cache: true,
		type: "POST",
		url: "<%=path%>/agent?p0=A-bigboss-search&p1=agentist1&p2="+pageIndex+"&p4=&p3=tojson" ,
		async: true,
			...
			for(var i = 0; i < json.length-1; i++) {
			   var online="";
				 ...
				 content +='<tr class="text-c">'
					...
					+'<td>'+json[i].id+'</td>'
					+'<td>'+json[i].agent_username+'</td>'
					+'<td>'+json[i].agent_password+'</td>'
					+'<td>'+json[i].agent_channelcode+'</td>'
					+'<td>'+json[i].agent_name+'</td>' 
					+'<td>'+json[i].agent_url+'</td>' 
					+'<td>'+json[i].totle_money+'</td>'
					+'<td>'+json[i].ablew_money+'</td>'
					+'<td>'+json[i].agent_pay+'</td>' 
					+'<td>'+json[i].agent_payname+'</td>' 
					+'<td>'+json[i].fencheng_u+'</td>' 
					+'<td>'+json[i].fencheng_v+'</td>' 
					
					
					
					+'<td><button type="button" class="btn btn-success radius" onclick="system_category_add(\'渠道商修改\',\'<%=path%>/ar?a=A-boss-mod&b=agent_modbf1&c='+json[i].id+'\',\'600\',\'600\')"   name=""><i class="Hui-iconfont">&#xe60c;</i>修改</button><br><br><button type="button" class="btn btn-danger radius"  onclick=shanchu(\''+json[i].id+'\')   name=""><i class="Hui-iconfont">&#xe609;</i>删除</button></td>'
...
function shanchu(id){
	$.ajax({
		type:'POST',
		url: '<%=path%>/ar?p0=A-boss-delete&p1=agent_delete&p2='+id,
		success: function(data){
