	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<c:forEach var="map" items="${reList}" varStatus="status">
			<tr class="text-c">
				...
				<td>${map['nickname']}</td>
				<td>${map['type']}</td>
				<td> <fmt:formatNumber type="number" value="${map['money']}" pattern="0.00" maxFractionDigits="2"/>(Y币)</td>
				<!-- <td>已结算</td> -->
				<td>${map['time']}</td>
		...
		<tr>
		<td colspan="8">当页收入（悦币）:${nodeValue}</td>
		</tr>	
		</tbody>
	</table>
...
function fresh_page(pageIndex) {
	...
	$.ajax({
		...
		url:"<%=path%>/ar?p0=A-boss-search&p1=income_table_search&p2="+pageIndex+"&p3="+startdate+"&p4="+enddate+"&p5=tojson&p6="+pp+"&p7=${param.p7}",
		async: true,
		...
		success: function(data) {
			...
			for(var i = 0; i < json.length-1; i++) {
				...
				+'<td>'+json[i].type+'</td>'
				+'<td>'+Number(json[i].money).toFixed(2)+'(Y币)</td>'
