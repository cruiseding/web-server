...
<div class="page-container">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<c:forEach var="map" items="${reList}" varStatus="status">
			...
			<!-- 序号、用户ID、头像、昵称、年龄、城市、个性签名、相册、自拍照、审核状态、审核时间、拒绝理由、操作 -->
				...
				<td style="color:red;">${map['result'] }</td>
				<%-- <td>${map['time'] }</td> --%>
				<td>${map['refusal'] }</td>
				<td><a href="javascript:;"
					onclick="client_del(${map['id']})"
					style="text-decoration: none;color:red;" class="ml-5">删除</a>
				</td>
			</tr>
		</c:forEach>	
		</tbody>
...
function fresh_page(pageIndex) {
	//var name = document.getElementById("searchname").value;
		$.ajax({
			cache:true,
			type:"post",		
			url: "<%=path %>/ar?a=A-boss-search&b=renzheng_v_no&p3="+pageIndex+"&page=tojson&p4=&p5=",
			async: true,
			...
			success:function(data){
				...
				for(var i = 0;i<json.length-1;i++){
					content +='<div id="outerdiv" style="position:fixed;top:0;left:0;background:rgba(0,0,0,0.7);z-index:2;width:100%;height:100%;display:none;"><div id="innerdiv" style="position:absolute;"><img id="bigimg" style="border:5px solid #fff;" src="" /></div></div>' 
						    ...
						    +'<td><a href="javascript:;" onclick="system_category_edit(\'认证图片\',\'<%=path %>/ar?p0=A-boss-search&p1=renzheng_photosearch&p2='+json[i].id+'&p3=0\',\'600\',\'510\')"  ><span style="color:blue;">点击查看相册</span></a></td>'
						  
						    +'<td><img class="pimg" alt="" onclick="imgShow2(\''+json[i].selfie+'\')" src="'+json[i].selfie+'" style="width:80px"></td>'
							
						    
						    
						    +'<td style="color:red;">'+json[i].result+'</td>'
						    /* +'<td>'+json[i].time+'</td>' */
						    +'<td>'+json[i].refusal+'</td>'
						    +'<td><a href="javascript:;" onclick="client_del('+json[i].id+')"  ><span style="color:red;">删除</span></a></td>'
						    ;				
			}
...
function client_del(id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '<%=path%>/ar?p0=A-boss-delete&p1=renzheng_record_del&p2='+id,
			success: function(data){
...
function go(id,userid){
	...
	layer.confirm('确认输入以上内容？',function(index){ */
		...
		$.ajax({
			type:'POST',
			url: '<%=path%>/ar?p0=A-boss-mod&p1=renzheng_checknopass&p2='+id+'&p4='+userid,
			success: function(data){
				/*$(obj).parents("tr").remove();*/
				layer.msg('操作成功',{icon:1,time:1000});
				setTimeout(function () { 
