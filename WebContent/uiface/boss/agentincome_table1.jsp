	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<c:forEach var="map" items="${reList}" varStatus="status">
			<tr class="text-c">
				...
				<td>
				<c:choose>
				     <c:when test="${map['money_type']=='充值'}"><span style="color:green">用户充值</span></c:when>
		...
		<tr>
		<td colspan="8">当页收入（元）:${nodeValue}</td>
		</tr>	
		</tbody>
	</table>
...
function fresh_page(pageIndex) {
	...
	$.ajax({
		...
		url:"<%=path%>/agent?p0=A-boss-search&p1=agentpay_table_search&p2="+pageIndex+"&p3="+startdate+"&p4="+enddate+"&p5=tojson&p6="+pp+"&p7=${param.p7}",
		async: true,
		...
		success: function(data) {
			...
			for(var i = 0; i < json.length-1; i++) {
				var z = '';
				if(json[i].money_type=='充值'){
					z = '<span style="color:green">用户充值</span>';
				}else if(json[i].money_type=='提现'){
					z = '<span style="color:red">主播提现</span>';
				}
