...
<nav class="breadcrumb">
	...
	<span class="c-gray en">&gt;</span> 手动充值 
...
<div class="page-container">
	<div class="text-c">	
		<div class="mt-20">
		    <input type="text" class="input-text" style="width: 50px;" placeholder="ID号" id="searchtext" name="searchtext">
	...
	<p id="sum1">总扣除趣豆:${reList[0].sum}</p>
	
	<p id="sum2">总充值趣豆:${reList[0].nsum}</p>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<c:forEach var="map" items="${reList}" varStatus="status">
			...
			<!-- 序号、用户ID、用户昵称、充值方式、充值金额、爱豆数量、充值状态、充值时间、个人充值记录（个人充值记录：序号、用户ID、充值类型、充值金额、充值状态、时间） -->
				<td>${status.count}</td>
				<td>${map['recharge_name']}</td>
				<td>${map['user_id']}</td>
				<td>${map['nickname']}</td>
				<td>${map['recharge_num']}趣豆</td>
				
				<td>${map['recharge_time']}</td>
		...
		<%-- <tr>
		<td colspan="12">当页充值:${nodeValue}</td>
		</tr> --%>
		</tbody>
	</table>
	...
	$("#searchbtn2").click(function() {
		...
		if(searchtext == undefined){ //返回true 
		 ...
		 layer.confirm('确认要导出数据吗？', function(index) {
				window.open("<%=path%>/ar?p0=A-boss-search&p1=recharge_user2&p2="+startdate+"&p3=1&p4="+enddate+"&p5="+searchtext+"&p6=tojson");
...
function fresh_page(pageIndex) {
	...
	$.ajax({
		...
		url:"<%=path%>/ar?p0=A-boss-search&p1=recharge_user&p2="+startdate+"&p3="+pageIndex+"&p4="+enddate+"&p5="+searchtext+"&p6=tojson",
		async: true,
		...
		success: function(data) {
			...
			for(var i = 0; i < json.length-1; i++) {
				...
				content +='<tr class="text-c">'
					...
					+'<td>'+json[i].recharge_name+'</td>'
					+'<td>'+json[i].user_id+'</td>'
					+'<td>'+json[i].nickname+'</td>'
					
					+'<td>'+json[i].recharge_num+'趣豆</td>'
					+'<td>'+json[i].recharge_time+'</td>'
					+'</tr>';
					var a = Number(json[i].recharge_num);
					sum = sum+a;
			}
			/* content +='<tr>'
					+'<td  colspan="12"> 当页充值:'+sum+'</td>'
						+'</tr>'; */
