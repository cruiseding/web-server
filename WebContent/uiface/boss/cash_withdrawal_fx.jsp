...
<nav class="breadcrumb">
	...
	<span class="c-gray en">&gt;</span> 分享收益提现
...
<div class="page-container">
	  <div class="mt-20">
			...
			<input type="text" id="d244" onclick="WdatePicker({dateFmt:'yyyy'})" class="input-text Wdate" style="width:120px;"  />
		      ...
		      <input type="text" class="input-text" style="width: 150px;" placeholder="请输入用户昵称" id="searchtxtnc" name="searchtxtnc">		
		      <!-- <select id="tixian" name="tixian" > -->
		      <select class="input-text" style="width:150px" id="tixian" name="tixian">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<c:forEach var="map" items="${reList}" varStatus="status">
			<tr class="text-c">
				...
				<td>
					<c:choose>
						<c:when test="${map['status']=='提现成功'}"><span>提现成功</span></c:when>
						<c:when test="${map['status']=='提现失败'}"><span>提现失败</span></c:when>
						<c:when test="${map['status']=='提现中'}"><input  type="button" href="javascript:;" class="btn btn-success radius" onclick="response_money(this,${map['id']})" value="确定">
...
function fresh_page(pageIndex) {
	...
	var tixian=$("option:selected","#tixian").val();
	...
	$.ajax({
		...
		url:"<%=path%>/ar?p0=A-boss-search&p1=cash_withdrawal_fx&p2="+pageIndex+"&p3="+startdate+"&p4="+enddate+"&p5=tojson&p6="+pp+"&p7=0&p8="+searchtext+"&p9="+searchtxtnc+"&p10="+tixian,
		async: true,
		...
		success: function(data) {
			...
			for(var i = 0; i < json.length-1; i++) {
				...
				}else if(json[i].status=='提现成功'){
					z = '<span>提现成功</span>';
				}else if(json[i].status=='提现失败'){
					z = '<span>提现失败</span>';
				}
		...
		function jujue_money(obj,id){
			layer.confirm('拒绝提现给用户？',function(index){
				...
				$.ajax({
					type: 'POST',
					url: '<%=path%>/ar?p0=A-boss-mod&p1=jujue_money&p2='+id+'&p3=1',
					success: function(data){
...
function response_money(obj,id){
	layer.confirm('确认提现给用户？',function(index){
		...
		$.ajax({
			type: 'POST',
			url: '<%=path%>/ar?p0=A-boss-mod&p1=response_money&p2='+id+'&p3=1',
			success: function(data){
				/*$(obj).parents("tr").remove();*/
				layer.msg(data,{icon:1,time:1000});
				setTimeout(function () { 
...
function seek2(){
	...
	layer.confirm('确认要导出数据吗？', function(index) {
		window.open("<%=path%>/ar?p0=A-boss-search&p1=cash_withdrawal_fx2&p2=1&p3="+startdate+"&p4="+enddate+"&p5=tojson&p6="+pp+"&p7=0&p8="+searchtext+"&p9="+searchtxtnc+"&p10="+tixian);
