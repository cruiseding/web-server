...
public class AppointInoutUser_C_01066 extends InOutManager implements InOutFace {
	...
	public AppointInoutUser_C_01066(String[] arg, HttpServletRequest request, HttpServletResponse response)
	 ...
	 * @param arg
	 * 
	 *            arg[0]: A-user-search arg[1]: getqrcode arg[2]: userid
	 */
	private void getqrcode(String[] arg) {
		response.setContentType("image/jpeg");

		logDetector.send(DataType.basicType, "01156", "getqrcode: ", "start");
		try {
			// 获取海报背景图
			// String url = "http://12nm.cn/img/haibao/haibao1.jpg";
			String url = "http://" + JyGlobalConstant.getIp() + ":8090/img/qrcodebg.jpg";
			logDetector.send(DataType.basicType, "01156", "qrcodeUrl: ", url);
			// BufferedImage bkBfImg =
			// toBufferedImage(Toolkit.getDefaultToolkit().getImage(url));
			...
			// String qrcodeUrl =
			// "http://12nm.cn/uiface/user/invitation.jsp?userid="+arg[2];
			//
			String qrcodeUrl = request.getScheme() + "://" + request.getServerName()
					+ "/uiface/login?a=A-user-search&b=shareqrcode&c=" + arg[2];
			logDetector.send(DataType.basicType, "01156", "qrcodeUrl: ", qrcodeUrl);
			...
			logDetector.send(DataType.basicType, "01156", "getqrcode: ", "finished");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			logDetector.send(DataType.exceptionType, "01156", "getqrcode: ", "exception: " + e);
		}
	}
	...
	private void zhubo_online(String[] arg) throws SQLException, IOException, ServletException {
		...
		if (num > 0) {
			...
			logDetector.send("01160", "jsonadd 6:", jsonadd);
			inOutUtil.return_ajax(jsonadd);
			return;
		}

		String jsonadd = "{\"success\":\"3\"}";// 忙碌
		logDetector.send("01160", "jsonadd 3:", jsonadd);
		inOutUtil.return_ajax(jsonadd);
	}

	private void searchmyinfo(String[] arg) throws SQLException, IOException, ServletException {
		// TODO Auto-generated method stub
	...
	private void appointupbf(String[] arg) throws SQLException, IOException, ServletException {
		// TODO Auto-generated method stub
	...
	private void appointup(String[] arg) throws SQLException, IOException, ServletException {
		// TODO Auto-generated method stub
		String[] times = arg[6].split(",");
		int succ = 0;
		...
		if (balance >= (totle + process_price)) {
			for (int i = 0; i < times.length; i++) {
				...
				sql = "insert into appoint_list (user_id,touser_id,appointday,appoint_type,appointtimes,ap_type,ap_price,process_price,unit_price,toap_price,ap_time,start_time,end_time) values ('"
						+ arg[2] + "','" + arg[3] + "','" + arg[4] + "','" + arg[5] + "','" + times[i] + "','" + arg[7]
						+ "','" + totle + "','" + process_price + "','" + price + "','" + toap_price + "',now(),'"
						+ arg[4] + " " + times[i].split("-")[0] + ":00" + "','" + arg[4] + " " + times[i].split("-")[1]
						+ ":00" + "')";
	...
	private void appointadd(String[] arg) throws SQLException, IOException, ServletException {
		// TODO Auto-generated method stub
		String sql = "insert into appoint_set (user_id,appointday,appoint_type,appointtime_set) values ('" + arg[2]
				+ "','" + arg[3] + "','" + arg[4] + "','" + arg[5] + "')";
		logDetector.send(DataType.basicType, "01182", "sql", sql);
	...
	private void search_user_appointinfo(String[] arg) throws SQLException, IOException, ServletException {
		// TODO Auto-generated method stub apstatus 10 拒絕預約
		String sql = "select * from appoint_list where touser_id=" + arg[3] + " and appointday='" + arg[4]
				+ "' and apstatus!=10   ";
		logDetector.send(DataType.basicType, "01182", "sql", sql);
		list = sqlUtil.get_list(sql);
		inOutUtil.return_ajax(JsonUtil.listToJson(list));
	...
	// 快撩 arg[2]pages
	private void search_my_appointinfo(String[] arg)
		...
		String sql = "select * from appoint_set where user_id=" + arg[3] + " and appointday='" + date1 + "'";
		logDetector.send(DataType.basicType, "01182", "sql", sql);
		...
		sql = "select * from appoint_set where user_id=" + arg[3] + " and appointday='" + date2 + "'";
		logDetector.send(DataType.basicType, "01182", "sql", sql);
		list1 = sqlUtil.get_list(sql);
		if (list1.size() == 0) {
			Map<String, Object> rowData = new HashMap<String, Object>();
		...
		sql = "select * from appoint_set where user_id=" + arg[3] + " and appointday='" + date3 + "'";
		logDetector.send(DataType.basicType, "01182", "sql", sql);
		list1 = sqlUtil.get_list(sql);
		if (list1.size() == 0) {
			Map<String, Object> rowData = new HashMap<String, Object>();
		...
		sql = "select * from appoint_set where user_id=" + arg[3] + " and appointday='" + date4 + "'";
		logDetector.send(DataType.basicType, "01182", "sql", sql);
		list1 = sqlUtil.get_list(sql);
		if (list1.size() == 0) {
			Map<String, Object> rowData = new HashMap<String, Object>();
		...
		logDetector.send(DataType.basicType, "01182", "sql", JsonUtil.listToJson(list));
		inOutUtil.return_ajax(JsonUtil.listToJson(list));
	}
