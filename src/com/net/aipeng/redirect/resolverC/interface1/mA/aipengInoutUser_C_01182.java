...
public class aipengInoutUser_C_01182 extends InOutManager implements InOutFace {
	protected ArrayList<Map<String, Object>> list;
	protected ArrayList<Map<String, Object>> list1 = new ArrayList<Map<String, Object>>();
	...
	SqlManagerFace sqlmface = new aipengSqlUser_C_01182();

	public aipengInoutUser_C_01182(String[] arg, HttpServletRequest request, HttpServletResponse response)
	...
	/**
	 * 认证提交 ,,,,,,,, renzheng_submit arg[2] user_id arg[3] photo arg[4] nickname
	 * arg[5] age arg[6] city arg[7] signature ---个性签名 arg[8] photo_album --相册
	 * arg[9] selfie ---自拍照 arg[10] refusal ---认证状态
	 * 
	 * @param arg
	...
	private void renzheng_submit(String[] arg) throws ServletException, SQLException, IOException {
		...
		logDetector.send(DataType.basicType, "01165", "女神认证--sql:", sql);
		int success = sqlUtil.sql_exec(sql);
		logDetector.send(DataType.basicType, "01165", "女神认证--success:", success);

		String sql1 = sqlmface.addSqlface(1, arg);
		logDetector.send(DataType.basicType, "01165", "用户表修改女神认证状态--sql:", sql);
		int success1 = sqlUtil.sql_exec(sql1);
		logDetector.send(DataType.basicType, "01165", "用户表修改女神认证状态--success:", success);
	...
	private void pay_vip(String[] arg) throws SQLException, IOException, ServletException {
		...
		String sql = "";
		String endDate = "";
		logDetector.send(DataType.basicType, "01182", "pay_vip", sql);
		...
		arg[3] = endDate;
		logDetector.send(DataType.basicType, "arg[3]", "arg[3]", arg[3]);
	...
	private void sayhi(String[] arg) throws SQLException, IOException, ServletException {
		...
		String sql = "";
		sql = sqlmface.modSqlface(0, arg);
		logDetector.send(DataType.basicType, "sql1", "sql1", sql);
		int res = sqlUtil.get_int(sql);
		logDetector.send(DataType.basicType, "sayhi_res", "sayhi_res", res);
		if (res > 0) {
			sql = sqlmface.modSqlface(1, arg);
			logDetector.send(DataType.basicType, "sql2", "sql2", sql);
			success = sqlUtil.sql_exec(sql);
		} else {
			sql = sqlmface.modSqlface(2, arg);
			logDetector.send(DataType.basicType, "sql3", "sql3", sql);
			success = sqlUtil.sql_exec(sql);
		}
		if (success == 1) {
			String jsonadd = "{\"success\":\"1\"}";
...
//		int res=sqlUtil.sql_exec(sql);
	...
	private void sefujin(String[] arg) throws SQLException, IOException, ServletException {
		...
		logDetector.send("01182", "用户经纬度", sql);
		list = sqlUtil.get_list(sql);
		logDetector.send("01182", "用户经纬度", list);
		Double longitude = Double.parseDouble(list.get(0).get("longitude").toString());
		arg[9] = longitude + "";
		myjingdu = longitude;
		logDetector.send("01182", "用户经度", arg[9]);
		...
		logDetector.send(DataType.basicType, "01198", "附近查询-用户经纬度最大最小值-aa", aa + "");
		arg[4] = aa[0] + ""; // 纬度最小值
		arg[5] = aa[2] + ""; // 纬度最大值
		arg[6] = aa[1] + ""; // 经度最小值
		arg[7] = aa[3] + ""; // 经度最大值
		...
		// 升序查询附近的主播
		logDetector.send("01182", "arg[4],arg[5],arg[6],arg[7]", arg[4] + "   " + arg[5] + "    " + arg[6] + "     " + arg[7]);
		list1 = sqlUtil.get_list(sql);
		logDetector.send("01182", "list1", list1);
		int total = list1.size();
		logDetector.send("01182", "list1", total);
		pages = JyHelpManager.pages(arg[3], total);
		arg[3] = pages[2] + "";
		String ceshi3 = arg[3];
		sql = sqlmface.searchSqlface(2, arg);
		logDetector.send("01182", "sql", sql);
		list = sqlUtil.get_list(sql);
		logDetector.send("01182", "list", list);
		// l
		for (int i = 0; i < list.size(); i++) {
			lng = Double.parseDouble((list.get(i).get("longitude").toString()));
			lat = Double.parseDouble(list.get(i).get("latitude").toString());
			long juli = (long) computeDistance(myjingdu, myweidu, lng, lat);
			logDetector.send("01182", "与我之间的距离", juli);
			list.get(i).put("juli", juli);
			arg[0] = list.get(i).get("id").toString();
			sql = sqlmface.searchSqlface(3, arg);
			logDetector.send("01182", "list_sql", sql);
			int res = sqlUtil.get_int(sql);
			logDetector.send("01182", "res", res);
			if (res >= 1) {
				list.get(i).put("status", "1");
			} else {
				list.get(i).put("status", "0");
			}
			logDetector.send("01182", "添加成功", juli);
		}
...
//			log.send(DataType.basicType, "01182", "女神堂", list);
	...
	private void kuailiao(String[] arg) throws SQLException, IOException, ServletException {
		// TODO Auto-generated method stub
		String sql = sqlmface.searchSqlface(0, arg);
		logDetector.send(DataType.basicType, "01182", "sql", sql);
		int total = sqlUtil.get_int(sql);
		pages = JyHelpManager.pages(arg[2], total);
		list1 = new ArrayList<Map<String, Object>>();
		list = new ArrayList<Map<String, Object>>();
		if (arg[2].equals("0") || arg[2].equals("1")) {
			String ressql = "select  a.*,b.id,b.nickname,b.user_photo,b.gender,b.age "
					+ "from Ask_Chat as a,user_data as b	"
					+ "where a.status=0 and a.qiu_id=b.id order by time desc ";
			logDetector.send(DataType.basicType, "196", "快撩页面查询抢聊表内容，显示未抢聊-sql", ressql);
			list1 = sqlUtil.get_list(ressql);
		}
...
//		log.send(DataType.basicType, "01182", "kuailiao", ceshi3);
		...
		for (int i = 0; i < list.size(); i++) {
			list.get(i).put("liao_type", 0);
			list.get(i).put("price", 0);
			list.get(i).put("qiu_type", 0);
			list.get(i).put("ask_id", 0);
		}
		...
		if (list1.size() != 0) {
			...
			for (int i = 0; i < list1.size(); i++) {
				...
				sql = "select count(*) from blacklist_table where (user_id=" + arg[3] + " and black_id=" + arg[0]
						+ " and status=1) or (user_id=" + arg[0] + " and black_id=" + arg[3] + " and status=1)";
				logDetector.send(DataType.basicType, "196", "快撩页面查询抢聊表内容，显示未抢聊-sql", sql);
				int isblack = sqlUtil.get_countint(sql);
				if (isblack > 0) {

				} else {
					list1.get(i).put("liao_type", 1);
					list1.get(i).put("time", 0);
					list1.get(i).put("ask_id", list1.get(i).get("id"));
					list1.get(i).put("id", list1.get(i).get("qiu_id"));
					list.add(m, list1.get(i));
					m++;
				}
...
//		log.send(DataType.basicType, "myaidou", "myaidou", sql);
		list = sqlUtil.get_list(sql);
		logDetector.send(DataType.basicType, "01205", "查询主播礼物数量-sql ", list);
		if (list.size() > 0) {
...
//		log.send(DataType.basicType, "myaidou", "myaidou", list);
		...
		logDetector.send(DataType.basicType, "myaidou", "myaidou", JsonUtil.listToJson(list));
		inOutUtil.return_ajax(JsonUtil.listToJson(list));
	}
...
//		log.send(DataType.basicType, "vipstatus", "vipstatus", list);
	...
	private void se_systemset(String[] arg) throws SQLException, IOException, ServletException {
		...
		String sql = sqlmface.searchSqlface(0, arg);
		list = sqlUtil.get_list(sql);
		logDetector.send(DataType.basicType, "se_systemset", "se_systemset", list);
		inOutUtil.return_ajax(JsonUtil.listToJson(list));
	}
	...
	private void is_renzheng(String[] arg) throws SQLException, IOException, ServletException {
		...
		logDetector.send(DataType.basicType, "01165", "女神认证--is_renzheng:", is_renzheng);
	...
	private void search_user(String[] arg) throws SQLException, ServletException, IOException {
		...
		String sql = sqlmface.searchSqlface(0, arg);
		logDetector.send(DataType.basicType, "01198", "执行搜索功能", sql);
		list = sqlUtil.get_list(sql);
		inOutUtil.return_ajax(JsonUtil.listToJson(list));
	...
	private void se_black_list(String[] arg) throws SQLException, IOException, ServletException {
		// TODO Auto-generated method stub
		String sql = sqlmface.searchSqlface(0, arg);
		logDetector.send(DataType.basicType, "01182", "执行搜索功能", sql);
		list = sqlUtil.get_list(sql);
		inOutUtil.return_ajax(JsonUtil.listToJson(list));
	}
