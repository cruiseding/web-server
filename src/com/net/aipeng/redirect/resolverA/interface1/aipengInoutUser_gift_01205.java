...
import com.net.aipeng.redirect.resolverA.interface2.aipengSqlUser_gift_01205;

public class aipengInoutUser_gift_01205 extends InOutManager implements InOutFace {
	...
	SqlManagerFace sqlmface = new aipengSqlUser_gift_01205();

	public aipengInoutUser_gift_01205(String[] arg, HttpServletRequest request, HttpServletResponse response)
	...
	/**
	 * arg[0] A-user-add arg[1] ask_gift arg[2] 发起ID arg[3] 目标ID arg[4] 礼物ID
	 * 
	 * @param arg
	 * @throws IOException
	 * @throws SQLException
	...
	private void ask_gift(String[] arg) throws SQLException, IOException, ServletException {
		...
		logDetector.send("01205", "求赏 查询礼物信息： sql ", sql);
		Map<String, Object> giftInfo = sqlUtil.get_list(sql).get(0);

		// 查询发起用户信息
		sql = sqlmface.addSqlface(1, arg);
		logDetector.send("01205", "求赏 查询用户信息： sql ", sql);
		Map<String, Object> userInfo = sqlUtil.get_list(sql).get(0);
		String pc = giftInfo.get("price").toString();
		if (pc == null || pc.isEmpty())
			pc = "0";
		// 构建消息体，通过openfire发送到目标用户
		StringBuilder sb = new StringBuilder();
		sb.append(arg[3]).append("卍ask_gift,").append(userInfo.get("nickname")) // 昵称
				.append(",").append(userInfo.get("id")) // 用户ID
				.append(",").append(userInfo.get("user_photo")) // 头像
				.append(",").append(giftInfo.get("id")) // 礼物ID
				.append(",").append(giftInfo.get("img")) // 礼物图标
				.append(",").append(giftInfo.get("name")) // 礼物名
				.append(",").append(pc) // 礼物的价格
		;

		// 发送openfire消息
		String openfireMSG = sb.toString();
		logDetector.send("01205", "发送求赏openfire消息：", openfireMSG);
	...
	private void give_gift(String[] arg) throws SQLException, IOException, ServletException {
		...
		if (list == null || list.isEmpty())// 没有查询到礼物数据
		{
			...
			if (yue < giftPrice) // 2.判断余额
			{
				...
				String weeksql = "update user_data set week_inout=week_inout+" + arg[4] + " where id ='" + arg[2]
						+ "' or id='" + arg[3] + "'";
				...
				logDetector.send(DataType.specialType, "01165", "发送openfire消息内容 ", ids + "卍" + "give_gift," + name1 + ","
						+ phote1 + "," + name2 + "," + phote2 + "," + gift_photo);
				BizRenderTask send = new BizRenderTask(ids + "卍" + "give_gift," + name1 + "," + phote1 + "," + name2
						+ "," + phote2 + "," + gift_photo);
				send.run();
			}
		}
	...
	/*
	 * arg[0] A-user-search arg[1] reward_gift arg[2] 未使用
	 */
	// 打赏礼物 (聊天界面)
	private void reward_gift(String[] arg) throws SQLException, ServletException, IOException {
		...
		logDetector.send(DataType.basicType, "01205", "打赏礼物（list查询）sql", sql);
		list = sqlUtil.get_list(sql);
		logDetector.send(DataType.basicType, "01205", "打赏礼物（list查询）-list", list);

		sql = "select balance from user_data where id='" + arg[2] + "'";
		String balance = sqlUtil.get_string(sql);
	...
	/*
	 * arg[0] A-user-search arg[1] gift_record arg[2] 未使用 arg[3] 主播ID // 将修改为用户ID
...
//	    list1=sqlUtil.get_list(sql);
		...
		logDetector.send(DataType.basicType, "01205", "查询礼物数量-sql ", sql);
	...
	/**
	 * arg[0] A-user-search arg[1] gift_count arg[2] 未使用 arg[3] 用户ID arg[4]
	 * 0表示查询送出礼物，1表示查询收到礼物
	 * 
	 * @param arg
	 * @throws SQLException
