...
public class aipengInoutUser_196 extends InOutManager implements InOutFace {
	...
	SqlManagerFace sqlmface = new aipengSqlUser_196();
	JyHelpManager helpmanager = new JyHelpManager();

	public aipengInoutUser_196(String[] arg, HttpServletRequest request, HttpServletResponse response)
...
//			break;
	...
	private void withdraw_cash(String[] arg) throws SQLException, ServletException, IOException {
		...
		logDetector.send(DataType.basicType, "196", "提现-sql:", sql);
		int success = sqlUtil.sql_exec(sql);
		logDetector.send(DataType.basicType, "196", "提现-success:", success);

		String sql1 = sqlmface.addSqlface(1, arg);
		logDetector.send(DataType.basicType, "196", "用户表扣除提现金额-sql1:", sql1);
		int success1 = sqlUtil.sql_exec(sql1);
		logDetector.send(DataType.basicType, "196", "用户表扣除提现金额-success1:", success1);
	...
	private void gift_record(String[] arg) throws SQLException, IOException, ServletException {
		...
		logDetector.send(DataType.basicType, "01162", "查看礼物记录-sql", sql);
		...
		for (int i = 0; i < list.size(); i++) {
			...
			logDetector.send(DataType.basicType, "01162", "查看礼物记录-sql", sql);
			int b = sqlUtil.get_countint(sql);
			list.get(i).put("gift_num", b + "");
		}

		sql = sqlmface.searchSqlface(3, arg);
		logDetector.send(DataType.basicType, "01205", "查询主播礼物数量-sql ", sql);
		int giftNum = sqlUtil.get_int(sql);
	...
	private void find_qiangliao(String[] arg) throws SQLException, ServletException, IOException {
		...
		logDetector.send(DataType.basicType, "196", "快撩页面查询抢聊表内容，显示未抢聊-sql", sql);
		list1 = sqlUtil.get_list(sql);
		logDetector.send(DataType.basicType, "196", "快撩页面查询抢聊表内容，显示未抢聊-list", list1);

		ArrayList<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
		logDetector.send(DataType.basicType, "196", "快撩页面查询抢聊表内容，显示未抢聊-list", list1.size());
		for (int i = 0; i < list1.size(); i++) {
			logDetector.send(DataType.basicType, "196", "快撩页面查询抢聊表内容，显示未抢聊-list", arg);
			...
			logDetector.send(DataType.basicType, "196", "快撩页面查询抢聊表内容，显示未抢聊-sql", sql);
			int isblack = sqlUtil.get_countint(sql);
			if (isblack > 0) {
	...
	private void reward_gift(String[] arg) throws SQLException, ServletException, IOException {
		String sql = sqlmface.searchSqlface(0, arg);
		logDetector.send(DataType.basicType, "196", "打赏礼物（list查询）sql", sql);
		list = sqlUtil.get_list(sql);
		logDetector.send(DataType.basicType, "196", "打赏礼物（list查询）-list", list);
		inOutUtil.return_ajax(JsonUtil.listToJson(list));
	}
...
//	4。快撩页面,点击抢，进入一对1.将这条记录改为已抢
	private void update_yiqiang(String[] arg) throws SQLException, ServletException, IOException {
		...
		logDetector.send(DataType.basicType, "196", "快撩页面,更改抢聊状态sql", sql);
		// int c=sqlUtil.get_int(sql);
		int success = sqlUtil.sql_exec(sql);
		logDetector.send(DataType.basicType, "196", "快撩页面,更改抢聊状态-success", success);
		if (success > 0) {
			String jsonadd = "{\"success\":\"1\"}";
			inOutUtil.return_ajax(jsonadd);
		} else {
	...
	private void is_rob_chat(String[] arg) throws SQLException, ServletException, IOException {
		String sql = sqlmface.modSqlface(0, arg);
		logDetector.send(DataType.basicType, "196", "更改抢聊状态sql", sql);
		// int c=sqlUtil.get_int(sql);
		int success = sqlUtil.sql_exec(sql);
		logDetector.send(DataType.basicType, "196", "更改抢聊状态-success", success);
		if (success > 0) {
			String jsonadd = "{\"success\":\"1\"}";
			inOutUtil.return_ajax(jsonadd);
		} else {
	...
	private void isone_on_one(String[] arg) throws SQLException, ServletException, IOException {
		String sql = sqlmface.searchSqlface(0, arg);
		logDetector.send(DataType.basicType, "196", "是否进入1对一抢聊sql", sql);
		int c = sqlUtil.get_int(sql);
		logDetector.send(DataType.basicType, "196", "是否进入1对一抢聊-list", c);
		if (c > 0) {
			String jsonadd = "{\"success\":\"1\"}";
			inOutUtil.return_ajax(jsonadd);
		} else {
	...
	private void seek_chat(String[] arg) throws SQLException, ServletException, IOException {
		String sql = sqlmface.searchSqlface(0, arg);
		logDetector.send(DataType.basicType, "196", "求聊  查在线女主播sql", sql);
		list = sqlUtil.get_list(sql);
		logDetector.send(DataType.basicType, "196", "求聊  查在线女主播-list", list);
		inOutUtil.return_ajax(JsonUtil.listToJson(list));
	}
...
//	}
	...
	private void add_qiuliao(String[] arg) throws SQLException, IOException, ServletException {
		...
		int balance = sqlUtil.get_int(sql2);
		if (balance >= Integer.parseInt(arg[3])) {
			String sql = sqlmface.addSqlface(0, arg);
			logDetector.send(DataType.basicType, "01196", "身份认证-sql1:", sql);
			int success = sqlUtil.sql_exec(sql);
			if (success == 1) {
				String jsonadd = "{\"success\":\"1\"}";
				inOutUtil.return_ajax(jsonadd);
	...
	class MyThread implements Runnable {
		...
		public void run() {
			...
			logDetector.send(DataType.basicType, "01196", "add_qiuliao-sql1:", id);
			try {
				jpushClient.sendPush(payload);
			} catch (APIConnectionException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
				logDetector.send(DataType.basicType, "01196", "add_qiuliao-sql1:", e);
	...
	private void jguang() throws SQLException {
		...
		logDetector.send(DataType.basicType, "01196", "add_qiuliao-sql1:", sql);
		list = sqlUtil.get_list(sql);
	...
	private void identity_approve(String[] arg) throws SQLException, IOException, ServletException {
		...
		String sql1 = sqlmface.addSqlface(1, arg);
		logDetector.send(DataType.basicType, "01196", "用户表修改女神认证状态--sql:", sql);
		int success1 = sqlUtil.sql_exec(sql1);
		logDetector.send(DataType.basicType, "01196", "用户表修改女神认证状态--success:", success);
	...
	private void give_gift(String[] arg) throws SQLException, IOException, ServletException {
		...
		logDetector.send("01205", "give_gift() 查询余额 sql", sql);
		int yue = sqlUtil.get_int(sql);

		sql = sqlmface.addSqlface(8, arg);
		logDetector.send("01205", "give_gift() 查询礼物数据 sql ", sql);
		List<Map<String, Object>> list = sqlUtil.get_list(sql);
		if (list == null || list.isEmpty())// 没有查询到礼物数据
		{
			...
			if (yue < giftPrice) // 2.判断余额
			{
				...
				logDetector.send("01205", "give_gift() 修改用户余额 sql", sql);
				int success1 = sqlUtil.sql_exec(sql);
				logDetector.send("01205", "give_gift() 修改用户余额 结果 ", success1);

				sql = sqlmface.addSqlface(3, arg); // 4.修改主播收入
				logDetector.send("01205", "give_gift() 修改主播余额 sql", sql);
				int success2 = sqlUtil.sql_exec(sql);
				logDetector.send("01205", "give_gift() 修改用户余额 结果 ", success2);

				sql = sqlmface.addSqlface(-1, arg); // 5.用户支出记录
				logDetector.send("01205", "give_gift() 修改用户支出记录 sql", sql);
				int success3 = sqlUtil.sql_exec(sql);
				logDetector.send("01205", "give_gift() 修改用户支出记录 结果 ", success3);

				sql = sqlmface.addSqlface(0, arg); // 6.修改主播收入记录
				logDetector.send("01205", "give_gift() 修改主播收入记录 sql", sql);
				int success4 = sqlUtil.sql_exec(sql);
				logDetector.send("01205", "give_gift() 修改主播收入记录 结果 ", success4);

				sql = sqlmface.addSqlface(6, arg); // 7.添加礼物赠送记录
				logDetector.send("01205", "give_gift() 添加礼物赠送记录 sql", sql);
				int success5 = sqlUtil.sql_exec(sql);
				logDetector.send("01205", "give_gift() 添加礼物赠送记录 结果 ", success5);

				sql = sqlmface.addSqlface(7, arg); // 8.更新主播收到礼物数目
				logDetector.send("01205", "give_gift() 更新主播收到礼物数目 sql", sql);
				int success6 = sqlUtil.sql_exec(sql);
				logDetector.send("01205", "give_gift() 更新主播收到礼物数目 结果 ", success6);
				...
				inOutUtil.return_ajax(jsonadd);
				logDetector.send(DataType.specialType, "01205", "打赏红包_反馈消息 ：", responseStr);

				// 10.查询数据，发送openfire信息。
				sql = "select nickname,user_photo from user_data where id=" + arg[2];
				logDetector.send(DataType.specialType, "01205", "查询用户信息", sql);
				...
				logDetector.send(DataType.specialType, "01205", "查询主播信息", sql);
				...
				logDetector.send(DataType.specialType, "01205", "查询礼物图片", sql);

				String gift_photo = sqlUtil.get_string(sql);
				sql = "select id from user_data where is_online=1 ";
				logDetector.send(DataType.specialType, "01205", "查询服务期内所有在线用户", sql);
				...
				logDetector.send(DataType.specialType, "01165", "发送openfire消息内容 ", ids + "卍" + "send_liwu," + name1 + ","
...
//				send.run();
	...
	private void wholookme(String[] arg) throws SQLException, ServletException, IOException {
		String sql = sqlmface.searchSqlface(0, arg);
		logDetector.send(DataType.basicType, "196", "首页单个显示--谁看过我sql", sql);
		list = sqlUtil.get_list(sql);
		logDetector.send(DataType.basicType, "196", "首页单个显示--谁看过我", list);
	...
	private void wholikesme(String[] arg) throws SQLException, ServletException, IOException {
		String sql = sqlmface.searchSqlface(0, arg);
		logDetector.send(DataType.basicType, "196", "首页单个显示--谁喜欢我sql", sql);
		list = sqlUtil.get_list(sql);
		logDetector.send(DataType.basicType, "196", "首页单个显示--谁喜欢我", list);
	...
	private void whoilike(String[] arg) throws SQLException, ServletException, IOException {
		String sql = sqlmface.searchSqlface(0, arg);
		logDetector.send(DataType.basicType, "196", "首页单个显示--我喜欢谁", sql);
		list = sqlUtil.get_list(sql);
		logDetector.send(DataType.basicType, "196", "首页单个显示--我喜欢谁", list);
	...
	private void likeeachother(String[] arg) throws SQLException, ServletException, IOException {
		String sql = sqlmface.searchSqlface(0, arg);
		logDetector.send(DataType.basicType, "196", "首页单个显示--相互喜欢sql", sql);
		list = sqlUtil.get_list(sql);
		logDetector.send(DataType.basicType, "196", "首页单个显示--相互喜欢-list", list);
	...
	private void findall_together_guanzhu(String[] arg) throws SQLException, ServletException, IOException {
		String sql = sqlmface.searchSqlface(0, arg);
		logDetector.send(DataType.basicType, "196", "查询（相互喜欢）列表sql", sql);
		list = sqlUtil.get_list(sql);
		logDetector.send(DataType.basicType, "196", "查询（相互喜欢）列表-list", list);

		String sql1 = sqlmface.searchSqlface(1, arg);
		logDetector.send(DataType.basicType, "196", "查询(相互喜欢)列表", sql1);
		list1 = sqlUtil.get_list(sql1);
		logDetector.send(DataType.basicType, "196", "查询(相互喜欢)列表1", list1);
	...
	private void findall_beiguanzhu(String[] arg) throws SQLException, ServletException, IOException {
		String sql = sqlmface.searchSqlface(0, arg);
		logDetector.send(DataType.basicType, "196", "查询(谁喜欢我)被关注列表sql", sql);
		list = sqlUtil.get_list(sql);
		logDetector.send(DataType.basicType, "196", "查询(谁喜欢我)被关注列表-list", list);

		String sql1 = sqlmface.searchSqlface(1, arg);
		logDetector.send(DataType.basicType, "196", "查询(谁喜欢我)被关注列表", sql1);
		list1 = sqlUtil.get_list(sql1);
		logDetector.send(DataType.basicType, "196", "查询(谁喜欢我)被关注列表1", list1);
...
//	  }
	private void findall_visitor(String[] arg) throws SQLException, ServletException, IOException {
		String sql = sqlmface.searchSqlface(0, arg);
		logDetector.send(DataType.basicType, "196", "查询所有‘谁看过我’列表sql", sql);
		list = sqlUtil.get_list(sql);
		logDetector.send(DataType.basicType, "196", "查询所有‘谁看过我’列表-list0", list);
...
//		list.get(0).put("visitornum", visitornum);
		String sql1 = sqlmface.searchSqlface(1, arg);
		logDetector.send(DataType.basicType, "196", "查询所有‘谁看过我’列表sql", sql1);
		list1 = sqlUtil.get_list(sql1);
		logDetector.send(DataType.basicType, "196", "查询所有‘谁看过我’列表", list1);
...
//			 });
	...
	private void findall_guanzhu(String[] arg) throws SQLException, ServletException, IOException {
		String sql = sqlmface.searchSqlface(0, arg);
		logDetector.send(DataType.basicType, "196", "查询所有我的关注sql", sql);
		list = sqlUtil.get_list(sql);
		logDetector.send(DataType.basicType, "196", "查询所有我的关注-list", list);

		String sql1 = sqlmface.searchSqlface(1, arg);
		logDetector.send(DataType.basicType, "196", "查询所有我的关注sql1", sql1);
		list1 = sqlUtil.get_list(sql1);
		logDetector.send(DataType.basicType, "196", "查询所有我的关注-list1", list1);
...
//	}
	...
	private void add_guanzhu(String[] arg) throws SQLException, IOException, ServletException {
		...
		int a = sqlUtil.get_int(findsql);
		logDetector.send(DataType.basicType, "196", "查询关注：是否已有关联联系 --sql00000:", findsql);
		logDetector.send(DataType.basicType, "196", "查询关注：是否已有关联联系 --count(*) a:", a);

		String findsql1 = sqlmface.searchSqlface(1, arg);
		int a1 = sqlUtil.get_int(findsql1);
		logDetector.send(DataType.basicType, "196", "查询关注：是否已有关联联系 --sql11111:", findsql);
		logDetector.send(DataType.basicType, "196", "查询关注：是否已有关联联系 --count(*) a1", a1);
		if (a == 0 && a1 == 0) {
			// 添加
			String addsql2 = sqlmface.searchSqlface(6, arg);
			logDetector.send(DataType.basicType, "196", "修改访问时间---如果互相都无访问记录，添加access-user-list：：：", addsql2);
			...
			int success = sqlUtil.sql_exec(cancelsql3);
			if (success == 1) {
				String jsonadd = "{\"success\":\"1\"}";
				inOutUtil.return_ajax(jsonadd);
			} else {
			...
			int success = sqlUtil.sql_exec(cancelsql5);
			if (success == 1) {
				String jsonadd = "{\"success\":\"1\"}";
				inOutUtil.return_ajax(jsonadd);
			} else {
	...
	private void quxiao_guanzhu(String[] arg) throws SQLException, IOException, ServletException {
		...
		int a = sqlUtil.get_int(findsql);
		logDetector.send(DataType.basicType, "196", "取消关注：是否已有关联联系 --sql00000:", findsql);
		logDetector.send(DataType.basicType, "196", "取消关注：是否已有关联联系 --count(*) a:", a);

		String findsql1 = sqlmface.searchSqlface(1, arg);
		int a1 = sqlUtil.get_int(findsql1);
		logDetector.send(DataType.basicType, "196", "取消关注：是否已有关联联系 --sql11111:", findsql);
		logDetector.send(DataType.basicType, "196", "取消关注：是否已有关联联系 --count(*) a1", a1);
...
//		} else 
	...
	private void fangke(String[] arg) throws SQLException, IOException, ServletException {
		String findsql = sqlmface.searchSqlface(0, arg);
		int a = sqlUtil.get_int(findsql);
		logDetector.send(DataType.basicType, "196", "查询访客是否已经存在 user-access-findsql:", findsql);
		logDetector.send(DataType.basicType, "196", "查询访客是否已经存在 user-access-list:", a);

		String findsql1 = sqlmface.searchSqlface(1, arg);
		int b = sqlUtil.get_int(findsql1);
		logDetector.send(DataType.basicType, "196", "访客不存在access-user-findsql1:", findsql1);
		logDetector.send(DataType.basicType, "196", "访客不存在access-user-list:", list);
		if (a == 0 && b == 0) {
			// 如果互相都无访问记录，添加
...
//		}
	...
	private void my_visitor(String[] arg) throws SQLException, IOException, ServletException {
		// TODO Auto-generated method stub
		String sql = sqlmface.addSqlface(0, arg);
		logDetector.send(DataType.basicType, "196", "查询访客是否已经存在-sql1:", sql);

		list1 = sqlUtil.get_list(sql);
		logDetector.send(DataType.basicType, "01196", "查询到的集合长度：-数据库语句", list1.size());
	...
	private void find_my_visitor(String[] arg) throws SQLException, ServletException, IOException {
		String sql = sqlmface.searchSqlface(0, arg);
		logDetector.send(DataType.basicType, "196", "查询我的访客sql", sql);
		list = sqlUtil.get_list(sql);
		int count_list = list.size();
		logDetector.send(DataType.basicType, "196", "我的访客人数-count_list", count_list);

		list.get(0).put("visitor_num", count_list);

		logDetector.send(DataType.basicType, "196", "查询我的访客-list", list);
