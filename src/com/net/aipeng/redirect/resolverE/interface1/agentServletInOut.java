...
@WebServlet("/uiface/agent")
public class agentServletInOut extends HttpServlet {

	private static final long serialVersionUID = -1204545408335287562L;

	public agentServletInOut() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * if(arg[1].equals("agent_login")){
		 * 
		 * }else{
		 * 
		 * if (arg.length>=2 && arg[0].startsWith("A-boss") ) { if
		 * (request.getSession().getAttribute("channelcode") == null ||
		 * "".equals(request.getSession().getAttribute("channelcode") .toString())) {
		 * 
		 * String
		 * js="<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\">"
		 * + "<script type=\"text/javascript\">" +
		 * "window.top.location.replace(\"http://116.62.220.67:8091/uiface1/boss/agentLogin.jsp\");"
		 * + "</script>"; PrintWriter out = response.getWriter(); out.write(js);
		 * 
		 * return; }else{
		...
		try {
			switch (arg[0]) {
				...
				case "A-boss-search":
					inOutFace = new agentInoutBoss_01066(arg, request, response);
				...
				/*
				 * case "A-bigboss-search": inOutFace=new agentInoutbigBoss_01066(arg, request,
				 * response); inOutFace.searchface(); break; case "A-bigboss-add": inOutFace=new
				 * agentInoutbigBoss_01066(arg, request, response); inOutFace.addface(); break;
				 * case "A-bigboss-mod": inOutFace=new agentInoutbigBoss_01066(arg, request,
				 * response); inOutFace.modface(); break; case "A-bigboss-del": inOutFace=new
				 * agentInoutbigBoss_01066(arg, request, response); inOutFace.deleteface();
				 * break;
				 */
	...
	public String[] upfile(String[] arg, HttpServletRequest request, HttpServletResponse response)
		...
		switch (arg[1]) {
			...
			default: {
				...
				log.send(DataType.specialType, "01115", "澶翠俊鎭�:   ", a);
				a = a.replaceAll("'", "\\\\'");
				log.send(DataType.specialType, "01115", "澶翠俊鎭�:   ", a);
				goodinfo[7] = a;
